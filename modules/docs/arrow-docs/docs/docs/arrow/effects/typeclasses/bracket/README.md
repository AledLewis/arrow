...
fun openFile(uri: String): IO<Throwable, File> = IO { File(uri).open() }

fun closeFile(file: File): IO<Throwable, Unit> = IO { file.close() }

fun fileToString(file: File): IO<Throwable, String> = IO { file.toString() }
```
