...
interface IOParMap3 {
  ...
  fun <E, A, B, C, D> parMapN(
    ...
    fun complete(a: A, b: B, c: C) {
      conn.pop()
      val result: BIOResult<E, D> = try {
        BIOResult.Right(f(a, b, c))
      } catch (e: Throwable) {
    ...
    IORunLoop.startCancelable(BIOForkedStart(fc, ctx), connC) { resultC ->
      resultC.fold({ e ->
