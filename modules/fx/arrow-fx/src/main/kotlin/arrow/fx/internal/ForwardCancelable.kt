...
internal class ForwardCancelable {
  ...
  fun cancel(): CancelToken<ForIO> {
    fun loop(conn: IOConnection, cb: (BIOResult<Nothing, Unit>) -> Unit): Unit = state.value.let { current ->
      when (current) {
  ...
  companion object {
    ...
    private sealed class State {
      data class Empty(val stack: List<(BIOResult<Nothing, Unit>) -> Unit>) : State()
    ...
    private fun execute(token: CancelToken<ForIO>, stack: List<(BIOResult<Nothing, Unit>) -> Unit>): Unit =
      // TODO this runs in an immediate execution context in cats-effect
