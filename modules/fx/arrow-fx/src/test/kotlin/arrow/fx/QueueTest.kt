...
class QueueTest : UnitSpec() {
  ...
  init {
    ...
    fun tests(
      ...
      queue: (Int) -> IO<Throwable, Queue<IOPartialOf<Throwable>, Int>>
    ) {
      ...
      "$label - time out taking from an empty queue" {
        IO.fx {
          val wontComplete = queue(10).flatMap(Queue<IOPartialOf<Throwable>, Int>::take)
      ...
      "$label - create a shutdown hook completing a promise, then shutdown the queue, the promise should be completed" {
        IO.fx {
          val q = !queue(10)
          val p = !Promise<IOPartialOf<Throwable>, Boolean>(IO.concurrent())
      ...
      "$label - create a shutdown hook completing a promise twice, then shutdown the queue, both promises should be completed" {
        IO.fx {
          val q = !queue(10)
          val p1 = !Promise<IOPartialOf<Throwable>, Boolean>(IO.concurrent())
          val p2 = !Promise<IOPartialOf<Throwable>, Boolean>(IO.concurrent())
    ...
    { capacity -> Queue.bounded<IOPartialOf<Throwable>, Int>(capacity, IO.concurrent()).fix() })
  }
}
